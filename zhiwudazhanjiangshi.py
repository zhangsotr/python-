...
class Zombie(pygame.sprite.Sprite):
    ...
    def move_zombie(self):
        if self.live and not self.stop:
            ...
            if self.rect.x < -80:
                # 调用游戏结束方法
                MainGame().gameOver()
    # v2.0 判断僵尸是否碰撞到植物，如果碰撞，调用攻击植物的方法
    def hit_plant(self):
    ...
    def eat_plant(self,plant):
        ...
        # 2. 植物死亡后的状态修改，以及地图状态的修改
        if plant.hp <= 0:
            a = plant.rect.y // 80 - 1
            b = plant.rect.x // 80
            map = MainGame.maps_list[a][b]
            map.can_grow = True
            plant.live = False
            #修改僵尸的移动状态
            self.stop = False
...
class MainGame:
    ...
    def startGame(self):
        ...
        while True:
            ...
            # 将包含文字的表面加入到窗口中
            MainGame.window.blit(sf,(300,50))
            sf_help = self.get_surface_with_content('1.左键创建向日葵 2.右键创建豌豆射手'.format(MainGame.money), 26, 'kaiti',
                                                    (0, 0, 255))
            MainGame.window.blit(sf_help,(10,10))
            # v1.2 调用加载地图方法
            self.load_maps()
            # v1.3 调用事件处理的方法
            self.deal_events()
            ...
            # v1.8 调用展示僵尸的方法
            self.load_zombies()
            # v2.0 计数器增长，每数到100，调用初始化僵尸的方法
            MainGame.count_zombie += 1
            if MainGame.count_zombie == 300:
                self.init_zombies()
